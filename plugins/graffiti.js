...
module.exports = {
    name: 'graffiti',
    description: 'Generate graffiti wall text effect',
    aliases: ['graf', 'graffitiwall'],
    tags: ['maker'],
    command: /^\.?(graffiti|graf|graffitiwall)$/i,

    async execute(sock, m, args) {
        try {
            const input = args.join(' ');
            if (!input) {
                return await m.reply('ᴘʀᴏᴠɪᴅᴇ ᴛᴇxᴛ!\n\nᴜsᴀɢᴇ: .graffiti <text1> | <text2>');
            }
            ...
            await m.reply('⏳ ɢᴇɴᴇʀᴀᴛɪɴɢ ɢʀᴀꜰꜰɪᴛɪ ᴛᴇxᴛ...');

            const res = await axios.get(`https://api.bk9.dev/maker/ephoto-2?text=${encodeURIComponent(text)}&text2=${encodeURIComponent(text2)}&url=https://en.ephoto360.com/create-a-graffiti-text-effect-on-the-wall-online-665.html`);
            ...
            const caption = `┌─ム ɢʀᴀꜰꜰɪᴛɪ ᴛᴇxᴛ ᴇꜰꜰᴇᴄᴛ
│
│ ᪣ ᴛᴇxᴛ 1: ${text}
│ ᪣ ᴛᴇxᴛ 2: ${text2 || 'N/A'}
│
