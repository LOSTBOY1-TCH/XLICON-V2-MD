const LINK_REGEX = /(?:https?:\/\/|wa\.me\/|www\.|chat\.whatsapp\.com\/|t\.me\/|telegram\.me\/|discord\.gg\/|discord\.com\/|(?:youtube\.com|youtu\.be)\/|instagram\.com\/|(?:facebook\.com|fb\.com)\/|(?:x\.com|twitter\.com)\/|tiktok\.com\/|reddit\.com\/)[^\s]*|(?:(?:[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?\.)+[a-z]{2,}(?:\/[^\s]*)?)/i;
...
module.exports = {
    ...
    async onMessage(sock, m) {
        try {
            ...
            // FIX: Check if ANY mode is active, not just enabled flag
            const isAnyModeActive = store.enabled || store.kick || (store.maxWarns && store.maxWarns > 0);
    ...
    async execute(sock, m, args) {
        try {
            if (!m.isGroup) {
                return await m.reply('┌─ు ᴀɴᴛɪ ʟɪɴᴋ\n│\n│ ᪣ ɢʀᴏᴜᴘ ᴏɴʟʏ ᴄᴏᴍᴍᴀɴᴅ\n│\n╰─────────◆────────╯');
            }

            if (!m.isAdmin && !m.isOwner) {
                return await m.reply('┌─ු ᴀɴᴛɪ ʟɪɴᴋ\n│\n│ ᪣ ᴀᴅᴍɪɴs ᴏɴʟʏ\n│\n╰─────────◆────────╯');
            }
            ...
            if (rawBody.startsWith(prefix + 'antilinkkick')) {
                ...
                if (!sub || sub === 'status') {
                    ...
                    return await m.reply(`┌─ු ᴀɴᴛɪ ʟɪɴᴋ ᴋɪᴄᴋ\n│\n│ ᪣ sᴛᴀᴛᴜs: ${status}\n│\n╰─────────◆────────╯`);
                }
                if (sub === 'on') {
                    store.kick = true;
                    store.enabled = false;
